...
We recruited participants located in the US from Amazon Mechanical Turk (MTurk) through the CloudResearch platform \cite{litman2017turkprime} \lwt{in the 1st quarter of 2020}. \lwt{MTurk population skews towards younger and higher educated population \cite{berinsky2012evaluating}, but an ideal survey technique should be inclusive and serve people at all ages and at any education level well. Therefore,} we made our best efforts to align the participants' age and education level distribution to the \lwt{latest available} United States census estimates in 2018 \cite{census2018}. We randomly assigned them into two groups: the Likert group and the QV group. Participants in the Likert group had a median completion time of 11.5 minutes and received \$0.75, and those in the QV group received \$2.5 due to a longer study length, with a median completion time of 20 minutes 56 seconds.
...
\Cref{fig:exp1_image_flow} summarizes the experimental procedure. The experiment consisted of four steps: 1) demographic survey, 2) Likert or QV survey, 3) \tc{filler task}, and 4) donation task. We provide the complete experimental protocol in the supplementary materials.
...
\tc{Though there is limtied prior works on using donation as an preference indicator, we believe voluntary donation was a suitable task to collect the participants' truthful preferences for four reasons.} First, many prior work \cite{Xiao2019, benz2008people, gendall2010effect, hsieh2010pay, hsieh2016you} used donation as an indicator of participants' preferences. \tc{Second, voluntary donation, either online or offline, is ecologically valid. It is a task that occurs in real-life settings. Thirdly, it is intuitive that people donate to causes of their interests while the amount of donation reflected their values.} Lastly, donation does not require specialized knowledge and is simple to conduct online and at scale. We now describe how our donation mechanism worked.
...
\begin{figure}[htpb]
    ...
    \caption{
        ...
        \tc{After multiple design iterations from pretest and early pilots,}
...
We used Bayesian analysis to compare if the distribution of cosine similarity angle in the QV group significantly differs from that in the Likert group. The core concept of Bayesian analysis is updating and reallocating the belief as one gathered more information with the use of Bayes rules \cite{kruschke2010bayesian}. \lwt{We chose Bayesian analysis over null hypothesis significance testing (NHST) for three reasons. First, Bayesian analysis encourages the accrual of quantitative knowledge in a field by getting more precise estimates via successive studies, as Kay et al. \cite{kay2016researcher} pointed out as a key benefit for the HCI community. Since our research is addressing a new topic with a series of open questions, our results in the form of probability distributions of estimated effect sizes can act as priors that inform future research. Kay et al. \cite{kay2016researcher} suggested a second benefit that Bayesian shifts the conversation focus from ``did it work" to ``how strong is the effect." A probability distribution of estimated effect size from Bayesian plays a critical role when stakeholders perform cost-benefit analysis on deciding which surveys to use \cite{kay2016researcher}. While NHST can technically answer the latter question with a single effect size estimate and confidence interval, the NHST logic relegates this information as secondary to the p-value and under-emphasizes it. Lastly, multiple works have criticized that NHST is intention-based \cite{ioannidis2005most, kruschke2010bayesian}, meaning that the choice of critical p-value is dependent on researchers' intentions, such as those during sample size adjustments and pre-selection of hypotheses. Bayesian analysis avoids such a pitfall \cite{kruschke2010bayesian}.}  
