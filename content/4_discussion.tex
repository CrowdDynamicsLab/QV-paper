...
While much prior work used binding voluntary donations with real monetary consequences to elicit participants' true preferences \cite{zawojska2015re, getzner2000hypothetical, ready2010using, benz2008people, gendall2010effect}, we acknowledge the limitations of such an approach, especially when we performed it with MTurk workers. For example, other factors, such as the prior experience of a charity, may impact one's donation amount. Thus, we surveyed the participants' impressions across the listed organizations after completing the donation task to mitigate possible confound. We found that across all experimental groups, participants had similar attitudes toward the chosen charities. Since we compare the two distances between one's donation amount (in proportion) and their survey responses, the effect of prior charity impressions cancels out. Another limitation is whether donation results on MTurk could be a reasonable estimation of MTurk workers' true preferences. A reasonable suspicion is that MTurk workers were less likely to donate in an MTurk task than in a real-life donation setting due to their more substantial incentives to earn money. Based on the assumption that MTurk workers would reduce their donation amount equally across all causes, we minimized this limitation by focusing on the proportion of donations across the charities instead of using the absolute donation value. In other words, we were looking at how much one was willing to donate to a charity \textit{relative} to the other organizations.}
