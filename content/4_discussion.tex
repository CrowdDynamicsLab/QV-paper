...
\tc{
\subsection{Donation Limitations}
One limitation lies in the design of using donations to measure an individual's truthful preferences. 
There were limited prior work that used donations to elicit participants' actual preferences, but donating is an ecologically valid behavior expected in a daily scenario.
We are aware that donating to a specific organization can be difficult to represent an individual's \textit{exact} preference of a broader social cause.
To address this, we surveyed the participant's perception of these organizations and found no significant confound between their donation behavior and their responses (see Appendix X.)
In addition, we focused on the proportion of donations spread across the charities instead of using the exact donation value. In other words, we are looking at how much one is willing to donate to a charity \textit{relative} to the other organizations.
One could view the donation behaviors as an approximation of an individual's true belief, yet, the question of how accurate donation can infer an individual's exact preference is not the focus of this paper, but rather, how does QV reflect individuals behaviors.
}
...
While our experiment samples covered a wide range of ages and education level, we targeted the US population specifically due to limited resources. Prior studies have found that cultural background affected response patterns in Likert scale surveys \cite{davey2007one}. Thus, future research needs to explore if cultural background also impacts people's interaction with QV and whether our results hold under those circumstances.
}
...
Our experiments, like all other experiments conducted on MTurk, suffered the risk of recruiting uncommitted Turkers that do not complete the experiment carefully. \tc{More specifically, Turkers have a stronger motive to earn money then to contribute their time and effort supplying truthful responses.} To mitigate the risk, we implemented multiple tutorials, quizzes, and attention checks, filtered out responses with facetious free-form text responses, and introduced incentive-compatible tasks with financial incentives. Yet, we acknowledge the limitation that we cannot guarantee that all participants joined the study with good faith or participated in the incentive-compatible tasks with a rational mindset.
